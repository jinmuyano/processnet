...
func NewNetflow(processKeywords []string, serviceAddr map[string]string) (*Netflow, error) {
...
func (nf *Netflow) increaseProcessTraffic(proc *Process, length int64, side sideOption, addr string) error {
	...
	switch side {
	case inputSide:
		proc.IncreaseInput(length, addr, nf.ServiceAddr)
	case outputSide:
		proc.IncreaseOutput(length, addr, nf.ServiceAddr)
	}
	return nil
}
