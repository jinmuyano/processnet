...
func hex2ip(hexstr string) (string, string) {
	...
	if len(hexstr) != 8 {
		// err := "parse error"
		// fmt.Println(hexstr, "parse error")
		// return ip, err
		// Ipv6
		hex = hexstr[len(hexstr)-8:]
	} else {
		hex = hexstr
	}

	i1, _ := strconv.ParseInt(hex[6:8], 16, 0)
	i2, _ := strconv.ParseInt(hex[4:6], 16, 0)
	i3, _ := strconv.ParseInt(hex[2:4], 16, 0)
	i4, _ := strconv.ParseInt(hex[0:2], 16, 0)
...
func netstat(t string) ([]*ConnectionItem, error) {
	...
	data, err := parseNetworkLines(t) //读取/proc/net/tcp,转列表
	if err != nil {
		return nil, err
	}

	for _, line := range data {
		pp := getConnectionItem(line) // 解析每一行
		if pp == nil {
			continue
		}
...
func getConnectionItem(line string) *ConnectionItem { //解析/proc/net/tcp文件
	// local ip and port
	...
	// ignore local listenning records,0.0.0.0代表本地流量
